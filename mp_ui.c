...
displayDayOfWeek (int nDay)
{
    char aDayNames[7][CHR_LIMIT] = {
        ...
        FORE_BLUE "Saturday " COLOR_RESET
        };
        ...
        else
        {
            ...
            do
            {
                ...
                bConfHereAndNow = isConfActiveNow(nConf, nLocation, nDay,
                                                  nPeriod);
                if (bConfHereAndNow && !aConfGiftedToday[nConf])
                {
...
fileMenu (int x, int y)
{
    ...
    char sSaveDetails[CHR_LIMIT]; // Concatenated to sSaveFileDesc
    ...
    for (i = 0; i <= 4; i++)
    {
        ...
        if (pFile != NULL)
        {
            ...
            sprintf(sSaveDetails, "%2d/%-2d (%s), Y%d",
                    toMonth(nDay), toDayOfMonth(nDay),
                    (nPeriod==AFTERNOON) ? "Afternoon" : "Evening",
                    nYen);
            strcat(sSaveFileDesc, sSaveDetails);
            fclose(pFile);
...
*/
void
displayConfStats (int aConfPts[], int aConfGifted[], int aConfGiftedToday[])
{
    int nConfPts;
    int nConfLv;
    int nConfToNextLv;
    ...
    for (int nConfidant = SOJIRO; nConfidant < AMOUNT_CONFIDANTS; nConfidant++)
    {
        ...
        copyConfName(sConfName, nConfidant);
        if (aConfGiftedToday[nConfidant] == true)
        {
            printf(FORE_RED);
        }
        else if (aConfGifted[nConfidant] == true)
        {
            printf(FORE_YELLOW);
        }
        printf("%8s" COLOR_RESET, sConfName);
...
displayStatScreen (int bStatViewToggle, int aConfPts[], int aStatPts[],
                   int aConfGifted[], int aConfGiftedToday[])
{
    if (bStatViewToggle)
        displaySocialStats(aStatPts);
    else
        displayConfStats(aConfPts, aConfGifted, aConfGiftedToday);
}
...
refreshScreen (int nLocation, int nDay, int nPeriod, int nYen, int aConfPts[],
               ...
               int aConfGiftedToday[], int nHighlight, int bStatViewToggle)
{
    system("cls");
    displayMap();
    markMap(nLocation, nDay, nPeriod, aConfPts, aConfGiftedToday);
    displayFooter("");
