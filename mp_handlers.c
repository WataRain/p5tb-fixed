...
handleConfAct (int nAction, int *pYen, int *pLentMoney, int aItems[],
               int aConfPts[], int aConfGifted[], int aConfGiftedToday[],
               int aStatPts[], int *pDoRefresh, int *pGiftedConfidant)
{
        ...
        if (nConfActType == GIVE_GIFT)
        {
            if (aConfGifted[nConf] == false)
            {
                ...
                printf("| Since you gave %s a gift, %s will now be colored\n| ",
                        sConf, sConf);
                printf(FORE_RED "red" COLOR_RESET " on the confidant stat"
                       " screen today,\n| and " FORE_YELLOW);
                ...
                aConfGiftedToday[nConf] = true;
                *pDoRefresh = true; // update confidant stat screen
                *pGiftedConfidant = true; // makes confidant action unavailable
            }
            else
            {
    ...
    if (nConfActType == SPEND_TIME)
    {
        ...
        if (toConf(nAction) != SADAYO || pay(100, pYen))
        {
            ...
            addStatPts(randInt(KINDNESS, KNOWLEDGE), randInt(1, 2), aStatPts);
            /*
...
handleSaveLoad (int nHighlight, int *pLocation, int *pDay, int *pPeriod,
                ...
                int *pDoRefresh, int *pSaveFileLoaded)
{
    int nSaveNum;
    switch (nHighlight)
    {
