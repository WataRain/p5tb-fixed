...
void unlockSOJIRO()
{
	...
	printf("\n");
	pauseText();
	printf("Sojiro: Anyway, if you are able to spare some time to help out in the cafe in the \n");
...
void maxFUTABA()
{
	...
	printf("we'll find a way to still remain together as a group? Our phantom thieving may be\n");
...
void maxSUMIRE()
{
	...
	printf("	[2] I was too busy staring at you to notice your performance.\n");
		nOption = getOption();
	} while(nOption != 1 && nOption != 2);
	printf("\n");
	if (nOption == 1) {
...
// Note: this has been modified to account for Sojiro being confidant 0
// instead of characterNo 1
void triggerEvent(int characterNo,int type)
{
	switch (characterNo)
	{
		case 0: // Sojiro Sakura
		if (type == 1)
			unlockSOJIRO();
		else if (type == 2)
			maxSOJIRO();
